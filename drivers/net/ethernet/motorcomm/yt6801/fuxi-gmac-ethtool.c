...
static
size_t
xn_strlcpy(char * __restrict dst, const char * __restrict src, size_t dsize)
{
	const char *osrc = src;
	size_t nleft = dsize;
				       ...
				       struct ethtool_drvinfo *drvinfo)
{
    ...
    xn_strlcpy(drvinfo->driver, pdata->drv_name, sizeof(drvinfo->driver));
    xn_strlcpy(drvinfo->version, pdata->drv_ver, sizeof(drvinfo->version));
    xn_strlcpy(drvinfo->bus_info, dev_name(pdata->dev),
...
static int fxgmac_get_rxfh(struct net_device *netdev, struct ethtool_rxfh_param * erp)
{
...
//(*set_rxfh)(struct net_device *, struct ethtool_rxfh_param *, struct netlink_ext_ack *extack);
static int fxgmac_set_rxfh(struct net_device *netdev, struct ethtool_rxfh_param * erp, struct netlink_ext_ack *extack)
{
    /*const u32 *indir =
    const u8 *key = 
    const u8 hfunc = ;*/
...
static const struct ethtool_ops fxgmac_ethtool_ops = {
    ...
    */
    ...
    .get_rxfh		= fxgmac_get_rxfh,//int	(*get_rxfh)(struct net_device *, struct ethtool_rxfh_param *);
    .set_rxfh		= fxgmac_set_rxfh,//(*set_rxfh)(struct net_device *, struct ethtool_rxfh_param *, struct netlink_ext_ack *extack);
#endif
#if (FXGMAC_WOL_FEATURE_ENABLED)
